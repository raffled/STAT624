...
## Stat 624 High Performance Computing Class 		##
## V.4, Designed to match the data structure presented in Ch. 5 for 		##
## Map Reduce ##
...
call.data <-function(call.list, n.days){
	...
	vec.list <- lapply(ind, function(h){
		...
		num.calls <- unlist(lapply(1:len, function(s){length(call.list[[h]][[2]][[s]][[1]])}))
		day <- rep(call.list[[h]][[1]], sum(num.calls))
		id <- unlist(lapply(1:len, function(d){call.list[[h]][[2]][[d]][[1]]}))
		user.carrier <- unlist(lapply(1:len, function(d){call.list[[h]][[2]][[d]][[2]]}))
		to.who <- unlist(lapply(1:len, function(d){call.list[[h]][[2]][[d]][[3]]}))
		to.who.carrier <- unlist(lapply(1:len, function(d){call.list[[h]][[2]][[d]][[4]]}))
		length.call <- unlist(lapply(1:len, function(d){call.list[[h]][[2]][[d]][[5]]}))
		list(date=day, sender=id, sender.carrier=user.carrier, receiver=to.who, receiver.carrier=to.who.carrier, length.call=length.call) 
	})
...
phone.list.gen <- function(num.user=c(10, 10), num.carriers=2, lambda.out=5, 
	...
	call.list <- lapply(1:n.days, function(s){
		...
		day.list <- lapply(1:length(user.id), function(x){
			...
			list(user.id=rep(user.id[x], call.out.vec), carrier=rep(carrier.list[x], call.out.vec), 
				to.who=user.id[who.ind], to.who.carrier=carrier.list[who.ind], length.call=l.c.m)
		})
		list(day=s, day.list)
	})
